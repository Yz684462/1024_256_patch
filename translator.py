...
class AssemblyTranslator:
    ...
    def parse_address(self, addr_str: str) -> int:
        """Convert hexadecimal string (like 'afa') to integer."""
        return int(addr_str, 16)
    
    def extract_lines_in_range(self, dump_lines: List[str], start_addr: int, end_addr: int) -> List[str]:
        ...
        for line in dump_lines:
            ...
            try:
                addr = self.parse_address(addr_part)
            except ValueError:
                continue
    ...
    def process_address_range(self, dump_lines: List[str], start_offset: int, end_offset: int, range_num: int) -> str:
        ...
        extracted_lines = self.extract_lines_in_range(dump_lines, start_offset, end_offset)
    ...
    def compile_to_shared_library(self, asm_content: str) -> str:
        ...
        translated_file = f"translate_{self.thread_index}.s"
        ...
        output_file = f"translate_part{self.thread_index}.so"
        compile_cmd = [
            "gcc",
            "-march=rv64gcv_zba",
            translated_file,
